...
class BaseFTPClient(object):
    ...
    def __init__(self, ftp_target: str, ftp_port: int, ssl: bool=False, verbosity: bool=False) -> None:
        ...
        self.peer = "{}:{}".format(self.ftp_target, self.ftp_port)
        if ssl:
    ...
    def connect(self, retries: int=1) -> bool:
        ...
        for _ in range(retries):
            try:
                ...
                print_error(self.peer, "FTP Error while connecting to the server", error_code, verbose=self.verbosity)
    ...
    def login(self, username: str, password: str) -> bool:
        ...
        try:
            ...
            print_error(self.peer, "FTP Authentication Failed, Username: {}, Password: {}".format(username, password), error_code, verbose=self.verbosity)
        self.ftp_client.close()
        return False
    ...
    def get_content(self, remote_file: str) -> str:
        ...
        try:
            content = io.BytesIO()
            self.ftp_client.retrbinary("RET: {}".format(remote_file), content.write)
            return content.getvalue()
        except Exception as error_code:
            print_error(self.peer, "FTP Error while retrieving content", error_code, verbose=self.verbosity)
        return None
    ...
    def close(self) -> bool:
        ...
        try:
            ...
            print_error(self.peer, "FTP Error while closing connection", error_code, verbose=self.verbosity)
        return False
...
class FTPClient(BaseFTPClient):
    ...
    def ftp_create(self, target: str=None, port: int=None) -> BaseFTPClient:
        """ Create FTP client
        ...
        ftp_client = BaseFTPClient(ftp_target, ftp_port, ssl=self.ssl, verbosity=self.verbosity)
        return ftp_client
